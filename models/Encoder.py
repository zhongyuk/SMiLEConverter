...
class Encoder_Network(object):
    ...
    def _generator(self, z, dims, train_phase, num_iter, activation=tf.nn.relu, scope_name="generator"):
        ...
        with tf.name_scope(scope_name) as scope:
        	...
        	h_bnebd = tf.nn.batch_normalization(h_ebd, h_bnebd_avg, h_bnebd_var, h_bnebd_beta, h_bnebd_gamma)
        	...
        	h_bnz = tf.nn.batch_normalization(h_z, h_bnz_avg, h_bnz_var, h_bnz_beta, h_bnz_gamma, decay=0.9)
        	...
        	for index in range(N - 2):
        		...
        		h_bn = tf.nn.batch_normalization(h_conv_t, bn_avg, bn_var, bn_beta, bn_gamma, decay=0.9)
        		h = activation(h_bn, name='h_%d'%index)
        		utils.add_activation_summary(h)
